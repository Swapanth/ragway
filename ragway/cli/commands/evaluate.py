...
def _resolve_runner(pipeline: str) -> Callable[[str], Awaitable[object]]:
    ...
    return cast(Callable[[str], Awaitable[object]], getattr(module, "run"))


async def _run_eval(dataset: list[dict[str, object]], pipeline: str) -> dict[str, float]:
...
def evaluate_command(dataset: Path, pipeline: str) -> None:
    ...
    summary = asyncio.run(_run_eval(typed_payload, pipeline))
